...
| `edt_validate_query` | Validates a 1C query against the project's live metadata: syntax **and** semantics (unknown tables/fields, type errors), with positions. Takes the query text, or a module address (`modulePath`, optionally one `method`) – then the bridge pulls the `\|`-framed query literals out of the live module itself and validates each one. |
