---
title: "Команды"
description: "Справочник команд обвязки edt-bridge-mcp: вызов инструмента из командной строки, список инструментов, состояние моста и самообновление."
sidebar:
  label: Команды
  order: 5
---
...
Обвязка `edt-bridge-mcp` служит MCP-клиенту stdio-сервером. Тем же исполняемым файлом мост вызывают и прямо из командной строки: посмотреть, поднят ли он, какие инструменты отдаёт и что отвечает один из них. Ради такой проверки поднимать агента не нужно.

Без команды обвязка работает как MCP-сервер – так её запускает клиент. Команды ниже адресуют тот же мост.

Код возврата различает два разных исхода. **1** – вызов не состоялся: мост не поднялся или аргументы неверны. **2** – инструмент отработал и вернул ошибку. В скриптах это разница между "мост лежит" и "мост жив, но задача не выполнена".

## Без команды: запуск MCP-сервера

stdio-обвязка MCP для плагина edt-bridge к 1С:EDT: пересылает запросы в запущенную EDT, а если ни одной нет – поднимает headless.
...
| `--workspace WORKSPACE` | рабочая область EDT для автозапуска headless |
...
Передать рабочую область клиентской EDT: остановить headless, дождаться, пока его процессы действительно исчезнут, и запустить окно EDT на той же рабочей области. Сама обвязка headless при этом не поднимает.
