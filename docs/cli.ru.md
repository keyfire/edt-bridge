...
Обвязка `edt-bridge-mcp` – это не только stdio-сервер для MCP-клиента. Тем же исполняемым файлом можно вызвать мост прямо из командной строки: посмотреть, поднят ли он, какие инструменты отдаёт и что отвечает конкретный из них. Это удобнее, чем поднимать агента, когда нужно всего лишь проверить, живо ли всё.
...
Передать воркспейс клиентской EDT: остановить headless, дождаться, пока его процессы действительно исчезнут, и запустить окно EDT на том же воркспейсе. Сама обвязка headless при этом не поднимает.
