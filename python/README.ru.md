...
Обвязка обновляет себя распаковкой, а не установщиком: качает wheel-пакет с PyPI (либо копирует пакет из
...
Внешние пакеты, установленные в окружение обвязки, могут добавлять свои инструменты MCP –
дом для того, чему не место в публичном репозитории (справочные материалы под чужой
лицензией, инструменты, завязанные на внутренний сервис). Обвязка находит их через группу
точек расширения `edt_bridge.tools`, перечисляет их инструменты рядом с инструментами моста
и выполняет сама – они отвечают даже без запущенной EDT.
...
`EDT_BRIDGE_NO_PLUGINS=1` выключает поиск плагинов. Контракт объявления для авторов –
на [странице установки](https://docs.keyfire.ru/edt-bridge/ru/install#плагины-обвязки).
