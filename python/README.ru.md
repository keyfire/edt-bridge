...
Обвязка обновляет себя распаковкой, а не установщиком: качает колесо с PyPI (либо копирует пакет из
рабочей копии, указанной в `--from`) и заменяет пакет в `site-packages` одной стандартной
библиотекой. Ни pip, ни pipx, ни бэкенда сборки – и это важно, потому что pipx 1.15 создаёт venv
...
заменить, да и не нужно – стаб при следующем запуске возьмёт тот код, который лежит в
