...
| `edt_infobase_config_state` | Whether an infobase actually **runs** the configuration it holds. Sessions execute the *database* configuration, a separate thing from the one being edited, so changes that are loaded but not applied leave every session on the previous code – while a project-to-infobase comparison reports them as equal. Answered through `ibcmd` by dumping both configurations and comparing them, addressing the infobase by file path or DBMS coordinates, so a clustered infobase needs no cluster access. |
