...
| `edt_add_method` · `edt_delete_method` | Add or delete a procedure/function in a module's BSL, both model-guided. The insert refuses any result that would not re-parse cleanly; the cut takes adjacent doc comments with it and its dry-run returns the exact removed text (`force` required – deleting code is destructive). |
...
| `edt_create_extension` · `edt_create_external_object` | Start a project. A configuration **extension** against a base project via `IExtensionProjectManager`, stamping name prefix and purpose (Customization·AddOn·Patch) onto a fresh detached extension Configuration – or an **external data processor** project, the start of the "processor → .epf" cycle. |
